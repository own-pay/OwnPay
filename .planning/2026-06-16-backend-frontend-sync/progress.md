...
- 2. Extended field audit to checkout/public/email/error (audit_templates_all.php, since deleted): CLEAN — no missing-var mismatches. checkout.twig/checkout-status.twig vars provided via dynamic $data renderers; intent vars guarded by `is defined`/`??`. Twig strict_variables=true confirmed.

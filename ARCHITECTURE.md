...
* **Global SMS parsing templates**: `op_sms_templates` are platform-owned (global) — created and managed
  **only** in the All-Brands view (`SmsTemplateAdminController` resolves `getPlatformId()`; create/edit/
  delete/save-analysis are `requireGlobalView`-guarded, and the per-brand SMS Center hides the New/Edit/
  Delete/sandbox UI). A companion device's sender whitelist (`ConfigController::filterRules` →
  `SmsTemplateRepository::listActiveForDevice()`) includes the device's own + `NULL` + **platform**
  templates, so an All-Brands template applies to every device regardless of the brand it paired under.
  (Until 2026-06-28 this controller used `getActiveBrandId()`/`$mid = 0`, orphaning templates at
  `merchant_id = 0` so no device whitelist matched → the fail-closed gate dropped every SMS.)
...
* **All-Brands Device Scope (resolve-on-match, multi-brand)**: A device's brand scope is implicit in *where* it was paired — pairing from the All-Brands view stores it under the platform-owner id (`BrandContext::getWriteMerchantId()`), pairing inside a brand stores it under that brand. `SmsVerificationJob::run()` keeps brand-scoped SMS strictly within their own brand, but matches a **platform-scoped (all-brands) device's** SMS **globally**: by provider trx id via `TransactionRepository::forAllTenants()->findByProviderTrxId()`, then by amount + gateway via `findPendingMatchGlobal()`. On a match it **re-attributes** the `op_sms_parsed` row to the brand that owns the matched transaction (`SmsParsedRepository::rebindToBrand()` — the single sanctioned cross-tenant move, since `updateScoped()` deliberately forbids `merchant_id` changes) and completes the transaction + posts the ledger under that brand. So every brand is verified by **both** the all-brands device and its own device; a brand-scoped device never affects another brand.
* **Money-Safe Global Matching (money-critical)**: The global amount fallback (`findPendingMatchGlobal`) is attempted **only** when the SMS carries a `received_at` window, and returns a row **only when exactly one** brand has a matching pending transaction inside that window. An amount that is ambiguous across brands (or arrives with no time window) is **refused** — never auto-credited to a guessed brand. Trx-id matches are exact and unaffected.
* **Real-Time Pairing & Live Status (admin)**: `DeviceController::generateOtp` returns a DB-clock `generated_at` baseline; `GET /admin/devices/pairing-status?since=` reports the newest device paired since that baseline, driving the pairing dialog's automatic "device connected" state (poll-based, no manual refresh — consistent with §7's poll-only model). `GET /admin/devices/statuses` returns each device with an `online` flag derived on the **DB clock** (active **and** `last_heartbeat` within `PairedDeviceRepository::ONLINE_THRESHOLD_SECONDS` = 180s); the admin list renders every device and **freezes revoked devices** as offline — a display guarantee complementing the H-04 API-level block. Both endpoints are read-only within the authenticated `admin` middleware group.
* **Online status needs a companion heartbeat**: because `online` is true only while `last_heartbeat` is within 180s, the companion app posts `POST /api/mobile/v1/devices/heartbeat` periodically (`DeviceHeartbeat`, ~60s + on resume/cold-start) — without it a paired device flips to **Idle** after 3 minutes. A specific brand's device list and `statuses` endpoint also include the global All-Brands (platform) devices (`PairedDeviceRepository::listWithLiveStatusForBrand`), so a device paired from the All-Brands view shows under each brand it serves (not only in the All-Brands list).

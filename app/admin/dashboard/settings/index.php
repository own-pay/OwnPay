...
<div class="flex flex-col lg:flex-row gap-6">
    ...
    <div class="w-full lg:w-56 flex-shrink-0 overflow-x-auto lg:overflow-visible">
        <nav class="flex lg:flex-col gap-1 lg:gap-0 lg:space-y-1 lg:sticky lg:top-20 pb-2 lg:pb-0" id="settings-tabs">
            ...
            <!-- General -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink active" data-settings-tab="general">
            ...
            <!-- API & Security -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink" data-settings-tab="api">
            ...
            <!-- Appearance -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink" data-settings-tab="appearance">
            ...
            <!-- Team & Access -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink" data-settings-tab="team">
            ...
            <!-- Domains -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink" data-settings-tab="domains">
            ...
            <!-- System -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink" data-settings-tab="system">
            ...
            <!-- Activity -->
            <button class="op-settings-tab flex-shrink-0 lg:flex-shrink" data-settings-tab="activity">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" id="settings-cards-grid">
                <?php foreach ($tabs as $tabKey => $tabCards): ?>
                    ...
                    <div class="op-settings-card-item <?= $visible ? '' : 'hidden' ?> <?= $tabKey !== 'general' ? 'hidden' : '' ?>"
                         ...
                         data-route="<?= htmlspecialchars($card['route'], ENT_QUOTES) ?>"
                         data-nav="<?= htmlspecialchars($card['nav'], ENT_QUOTES) ?>">
...
<script nonce="<?= $csp_nonce ?? '' ?>">
    window.switchSettingsTab = function(btn, tabKey) {
    ...
    // Attach tab-switch handlers via addEventListener (CSP-safe)
    document.querySelectorAll('.op-settings-tab[data-settings-tab]').forEach(function(btn) {
        btn.addEventListener('click', function() {
            window.switchSettingsTab(this, this.dataset.settingsTab);
        });
    });

    // Attach card click handlers via addEventListener (CSP-safe)
    var siteAdminBase = '<?= addslashes($site_url . $path_admin) ?>';
    document.querySelectorAll('.op-settings-card-item[data-route]').forEach(function(card) {
        card.addEventListener('click', function() {
            load_content('Settings', siteAdminBase + '/' + this.dataset.route, this.dataset.nav);
        });
    });
    ...
    (function() {
        ...
        if (tabParam) {
            ...
            if (tabBtn) window.switchSettingsTab(tabBtn, tabParam);
        }
    })();
</script>
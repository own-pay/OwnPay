...
<div class="op-page-header">
    <div>
        <div class="op-page-pretitle">Payments</div>
        <h2 class="op-page-title">Transactions</h2>
    </div>
</div>
...
<div class="flex justify-center mb-4">
    <div class="op-card inline-block p-1">
        <div class="flex gap-1" id="statusTabs" role="tablist" aria-label="Transaction status filter">
            <button class="op-tab active" data-type="all" role="tab" aria-selected="true" type="button">All</button>
            <button class="op-tab" data-type="completed" role="tab" aria-selected="false" type="button">Completed</button>
            <button class="op-tab" data-type="pending" role="tab" aria-selected="false" type="button">Pending</button>
            <button class="op-tab" data-type="refunded" role="tab" aria-selected="false" type="button">Refunded</button>
            <button class="op-tab" data-type="canceled" role="tab" aria-selected="false" type="button">Canceled</button>
        </div>
    </div>
</div>
        ...
        <table class="op-table">
            <thead>
                <tr>
                    ...
                    <th>Customer</th>
                    <th class="hidden sm:table-cell">Gateway</th>
                    <th>Amount</th>
                    <th class="hidden md:table-cell">Net Amount</th>
...
<script nonce="<?= $csp_nonce ?? '' ?>" data-cfasync="false">
    ...
    function load_data_list(page = 1){
        ...
        document.querySelector(".table-data-list").innerHTML = apSkeletonRows(8);

        opFetch('transaction-list', { search_input, show_limit, tabType, page, filter_status, filter_start, filter_end })
            .then(res => {
                let html = '';
                if (res.status === 'true') {
                    ...
                    res.response.forEach(item => {
                        ...
                        if (item.status === 'completed') badgeClass = 'op-badge-success';
                        if (item.status === 'pending') badgeClass = 'op-badge-warning';
                        if (item.status === 'refunded') badgeClass = 'op-badge-info';
                        if (item.status === 'canceled') badgeClass = 'op-badge-danger';

                        let editAttr = allowEdit ? `class="cursor-pointer" onclick="load_content('Edit Transaction','<?php echo $site_url.$path_admin ?>/transaction/edit?t_id=${item.id}','nav-item-transaction')"` : '';
                        ...
                        html += `
                            <tr data-id="${item.id}">
                                ...
                                <td class="hidden sm:table-cell" ${editAttr}>${apEscapeHtml(item.gateway)}</td>
                                <td ${editAttr}>${item.amount}</td>
                                <td class="hidden md:table-cell" ${editAttr}>${item.net_amount}</td>
                                <td class="hidden lg:table-cell" ${editAttr}>${apEscapeHtml(item.trx_id)}</td>
                    ...
                    if (html === '') {
                        html = `<tr><td colspan="9">${apEmptyState(res.title, res.message)}</td></tr>`;
                    }
                    ...
                    document.querySelector(".table-data-list").innerHTML = `<tr><td colspan="9">${apEmptyState(res.title, res.message)}</td></tr>`;
    ...
    document.querySelectorAll('#statusTabs .op-tab').forEach(btn => {
        btn.addEventListener('click', function () {
            document.querySelectorAll('#statusTabs .op-tab').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-selected', 'false'); });

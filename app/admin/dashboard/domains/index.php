<?php
    ...
    if (!\OwnPay\Service\Auth\PermissionService::canAccessPage(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', $global_user_response['response'][0]['role'])) { http_response_code(403); exit('Access denied.'); }
?>

<div class="op-page-header">
    <div><div class="op-page-pretitle">Settings</div><h2 class="op-page-title">Domains</h2></div>
    <div class="flex items-center gap-2">
        <span class="global-loaderSpinner"></span>
        <span class="<?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'whitelist', $global_user_response['response'][0]['role']) ? '' : 'hidden' ?>">
            <button class="op-btn-primary" data-modal-target="modal-createItem" data-modal-toggle="modal-createItem">Whitelist Domain</button>
        </span>
    </div>
</div>
...
<div class="op-card">
    ...
    <div class="overflow-x-auto"><table class="op-table"><thead><tr><th class="w-8"><input class="op-checkbox select-all" type="checkbox"></th><th>Domain</th><th>Created Date</th><th>Status</th><th></th></tr></thead><tbody class="table-data-list"></tbody></table></div>
...
<div id="model-bulkAction" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" data-modal-backdrop="static">
    <div class="relative p-4 w-full max-w-md max-h-full"><div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
        ...
        <div class="p-4"><label class="op-label">Action <span class="text-red-500">*</span></label><select class="op-select" id="model-bulkActionID"><option value="" selected>Select a Action</option>
            <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'delete', $global_user_response['response'][0]['role']) ? '<option value="deleted">Delete Selected</option>' : '' ?>
            <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'edit', $global_user_response['response'][0]['role']) ? '<option value="activated">Activate Selected</option><option value="inactive">Inactive Selected</option>' : '' ?>
        </select></div>
...
<div id="modal-createItem" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" data-modal-backdrop="static">
    <div class="relative p-4 w-full max-w-lg max-h-full"><div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
        <div class="flex items-center justify-between p-4 border-b dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white">New Domain</h3><button type="button" class="op-modal-close" data-modal-hide="modal-createItem"><svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/></svg></button></div>
        <div class="p-4 space-y-4">
            <div><label class="op-label">Domain Name <span class="text-red-500">*</span></label><div class="flex"><span class="inline-flex items-center px-3 text-sm text-gray-500 bg-gray-100 border border-e-0 border-gray-300 rounded-s-lg dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">https://</span><input type="text" class="op-input rounded-s-none" name="domain-name" placeholder="Domain name"></div></div>
            <div>
                <label class="op-label">Status <span class="text-red-500">*</span></label>
                <div class="grid grid-cols-2 gap-3">
                    <label class="flex items-start gap-3 p-3 border rounded-lg cursor-pointer has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50 dark:border-gray-600"><input type="radio" name="domain-status" value="active" class="mt-1 w-4 h-4 text-primary-600" checked><div><div class="font-medium text-sm text-gray-900 dark:text-white">Active</div><div class="text-xs text-gray-500">Domain can initiate payments.</div></div></label>
                    <label class="flex items-start gap-3 p-3 border rounded-lg cursor-pointer has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50 dark:border-gray-600"><input type="radio" name="domain-status" value="inactive" class="mt-1 w-4 h-4 text-primary-600"><div><div class="font-medium text-sm text-gray-900 dark:text-white">Inactive</div><div class="text-xs text-gray-500">Domain can't initiate payments.</div></div></label>
                </div>
            </div>
        </div>
        <div class="flex justify-end gap-2 p-4 border-t dark:border-gray-700"><button type="button" class="op-btn-secondary" data-modal-hide="modal-createItem">Cancel</button><button type="button" class="op-btn-primary modal-createItem-btn">Create</button></div>
    </div></div>
</div>
...
<div id="modal-editItem" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" data-modal-backdrop="static">
    <div class="relative p-4 w-full max-w-lg max-h-full"><div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
        <div class="flex items-center justify-between p-4 border-b dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit Domain</h3><button type="button" class="op-modal-close" data-modal-hide="modal-editItem"><svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/></svg></button></div>
        <div class="p-4 space-y-4">
        ...
        <div class="flex justify-end gap-2 p-4 border-t dark:border-gray-700"><button type="button" class="op-btn-secondary" data-modal-hide="modal-editItem">Cancel</button><button type="button" class="op-btn-primary modal-editItem-btn">Save Changes</button></div>
    </div></div>
</div>
...
<script nonce="<?= $csp_nonce ?? '' ?>" data-cfasync="false">
    ...
    document.querySelector('.model-bulkAction-btn').addEventListener('click', function(){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            opFetch('domain-bulk-action', { actionID, selected_ids: JSON.stringify(selectedRows) }).then(response => {
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            opFetch('domains-delete', { ItemID }).then(response => {
        ...
        } else { show_action_confirmation_tab('btnDeleteItem-'+ItemID, 'Delete Domain', 'Delete', 'btn-danger'); }
    }

    function load_data_list(page = 1){
        currentPage = page;
        document.querySelector(".table-data-list").innerHTML = apSkeletonRows(5);
        opFetch('all-domain-list', { search_input: document.querySelector('.search_input').value, show_limit: document.querySelector('.show_limit').value, page, filter_status: document.getElementById('filter-status').value, filter_start: document.getElementById('filter-created-from').value, filter_end: document.getElementById('filter-created-until').value }).then(res => {
            let html = '';
            if (res.status === 'true') {
                let allowEdit = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'edit', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                let allowDelete = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'delete', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                res.response.forEach(item => {
                    ...
                    html += `<tr data-id="${item.id}"><td><input class="op-checkbox rowCheckbox" type="checkbox"></td><td ${redirectEdit}>${item.domain}</td><td ${redirectEdit}>${item.created_date}</td><td ${redirectEdit}><span class="${badge}">${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</span></td>
                    <td class="text-end"><button class="op-btn-secondary text-xs" data-dropdown-toggle="dd-${item.id}">Actions</button><div id="dd-${item.id}" class="hidden z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-36 dark:bg-gray-700"><ul class="py-2 text-sm"><li class="${allowEdit?'':'hidden'}"><a href="javascript:void(0)" ${redirectEdit} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">Edit</a></li><li class="${allowDelete?'':'hidden'}"><a href="javascript:void(0)" ${redirectDelete} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 btnDeleteItem-${item.id}">Delete</a></li></ul></div></td></tr>`;
                });
    ...
    function openEditModel(itemID){
        document.querySelector('.global-loaderSpinner').innerHTML = '<div class="op-spinner"></div>';
        opFetch('domains-info-byID', { ItemID: itemID }).then(response => {
            document.querySelector('.global-loaderSpinner').innerHTML = '';
            if (response.status === 'true') {
                const modal = document.getElementById("modal-editItem");
                modal.querySelector('input[name="domain-id"]').value = itemID;
                modal.querySelector('input[name="domain-name"]').value = response.domain || '';
                modal.querySelectorAll('input[name="domain-status"]').forEach(input => input.checked = input.value === response.istatus);
    ...
    document.querySelector('.modal-createItem-btn').addEventListener('click', function(){
        ...
        var domain_name = modal.querySelector('input[name="domain-name"]').value;
        var statusInput = modal.querySelector('input[name="domain-status"]:checked');
        var domain_status = statusInput ? statusInput.value : "";
        if(!domain_name || !domain_status){ apToast('error','Incomplete','Please fill in all required fields.'); return; }
        var btnEl = this; var btn = btnEl.innerHTML;
        btnEl.innerHTML = '<div class="op-spinner" style="width:16px;height:16px;border-width:2px"></div>';
        opFetch('create-domains', { domain_name, domain_status }).then(response => {
            closeAllModals(); modal.querySelectorAll('input[type="text"]').forEach(i=>i.value=''); modal.querySelectorAll('input[name="domain-status"]').forEach((r,i)=>r.checked=(i===0)); btnEl.innerHTML = btn;
            response.status === 'true' ? (apToast('success', response.title, response.message), load_data_list(1)) : apToast('error', response.title, response.message);
        }).catch(err => apToastError());
    });
    ...
    document.querySelector('.modal-editItem-btn').addEventListener('click', function(){
        const modal = document.getElementById("modal-editItem");
        var domain_id = modal.querySelector('input[name="domain-id"]').value;
        var domain_name = modal.querySelector('input[name="domain-name"]').value;
        var statusInput = modal.querySelector('input[name="domain-status"]:checked');
        var domain_status = statusInput ? statusInput.value : "";
        if(!domain_id || !domain_name || !domain_status){ apToast('error','Incomplete','Please fill in all required fields.'); return; }
        var btnEl = this; var btn = btnEl.innerHTML;
        btnEl.innerHTML = '<div class="op-spinner" style="width:16px;height:16px;border-width:2px"></div>';
        opFetch('domains-edit', { domain_id, domain_name, domain_status }).then(response => {
            closeAllModals(); modal.querySelectorAll('input[type="text"]').forEach(i=>i.value=''); btnEl.innerHTML = btn;
            response.status === 'true' ? (apToast('success', response.title, response.message), load_data_list(1)) : apToast('error', response.title, response.message);
        }).catch(err => apToastError());
    });
</script>

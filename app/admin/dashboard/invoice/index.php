...
<div class="op-page-header">
    ...
    <div class="flex items-center gap-3">
        ...
        <span onclick="load_content('Create Invoice','<?php echo $site_url.$path_admin ?>/invoice/create','nav-item-invoice')" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'create', $global_user_response['response'][0]['role']) ? '' : 'hidden' ?>">
        ...
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
            ...
            <div class="p-4">
                ...
                <select class="op-select" id="model-bulkActionID">
                    ...
                    <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'delete', $global_user_response['response'][0]['role']) ? '<option value="deleted">Delete Selected</option>' : '' ?>
                </select>
            </div>
            ...
            <form action="" class="form-manageStatus">
                ...
                <div class="p-4">
                    ...
                    <select class="op-select" name="status" id="invoice-status">
                        <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'edit', $global_user_response['response'][0]['role']) ? '<option value="paid">Paid</option>' : '' ?>
                        <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'edit', $global_user_response['response'][0]['role']) ? '<option value="unpaid">Unpaid</option>' : '' ?>
                        <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'edit', $global_user_response['response'][0]['role']) ? '<option value="refunded">Refund</option>' : '' ?>
                        <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'edit', $global_user_response['response'][0]['role']) ? '<option value="canceled">Cancel</option>' : '' ?>
                    </select>
                </div>
                <div class="flex items-center justify-between p-4 border-t border-gray-200 dark:border-gray-700">
    ...
    function load_data_list(page = 1){
        ...
        opFetch('invoice-list', { search_input, show_limit, tabType, page, filter_status, filter_start, filter_end })
            .then(res => {
                let html = '';
                if (res.status === 'true') {
                    let allowEdit = <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'edit', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                    let allowDelete = <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'delete', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;

...
<div class="op-page-header">
    <div><div class="op-page-pretitle">Settings</div><h2 class="op-page-title">Addons</h2></div>
    <div class="flex items-center gap-2">
        <span class="global-loaderSpinner"></span>
        <span class="<?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'addons', 'create', $global_user_response['response'][0]['role']) ? '' : 'hidden' ?>">
            <button class="op-btn-primary" data-modal-target="modal-createItem" data-modal-toggle="modal-createItem">New Addon</button>
        </span>
    </div>
</div>
...
<div class="op-card">
    ...
    <div class="overflow-x-auto"><table class="op-table"><thead><tr><th class="w-8"><input class="op-checkbox select-all" type="checkbox"></th><th>Addon</th><th>Status</th><th></th></tr></thead><tbody class="table-data-list"></tbody></table></div>
...
<div id="model-bulkAction" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" data-modal-backdrop="static">
    <div class="relative p-4 w-full max-w-md max-h-full"><div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
        ...
        <div class="p-4"><label class="op-label">Action <span class="text-red-500">*</span></label><select class="op-select" id="model-bulkActionID"><option value="" selected>Select a Action</option>
            <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'addons', 'delete', $global_user_response['response'][0]['role']) ? '<option value="deleted">Delete Selected</option>' : '' ?>
            <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'addons', 'edit', $global_user_response['response'][0]['role']) ? '<option value="activated">Activate Selected</option><option value="inactivated">Inactive Selected</option>' : '' ?>
        </select></div>
...
<div id="modal-createItem" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" data-modal-backdrop="static">
    <div class="relative p-4 w-full max-w-lg max-h-full"><div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
        <div class="flex items-center justify-between p-4 border-b dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white">New Addon</h3><button type="button" class="op-modal-close" data-modal-hide="modal-createItem"><svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/></svg></button></div>
        <div class="p-4">
            <label class="op-label">Addon <span class="text-red-500">*</span></label>
            <select class="js-select op-select" name="addon" data-search="true" data-remove="true" required>
                <?php
                    ...
                    foreach ($addonDirs as $dir) {
                        ...
                        $addonObj = new $class();
                        $addons[$slug] = $addonObj->info();
                    }
                    foreach ($addons as $slug => $addon) {
                        echo '<option value="'.$slug.'">'.$addon['title'].'</option>';
                    }
                ?>
            </select>
        </div>
...
<script nonce="<?= $csp_nonce ?? '' ?>" data-cfasync="false">
    ...
    document.querySelector('.model-bulkAction-btn').addEventListener('click', function(){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            opFetch('addons-bulk-action', { actionID, selected_ids: JSON.stringify(selectedRows) }).then(response => {
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            opFetch('addons-delete', { ItemID }).then(response => {
        ...
        } else { show_action_confirmation_tab('btnDeleteItem-'+ItemID, 'Delete Addon', 'Delete', 'btn-danger'); }
    }

    function load_data_list(page = 1){
        currentPage = page;
        document.querySelector(".table-data-list").innerHTML = apSkeletonRows(5);
        opFetch('addons-list', { search_input: document.querySelector('.search_input').value, show_limit: document.querySelector('.show_limit').value, page, filter_status: document.getElementById('filter-status').value, filter_start: document.getElementById('filter-created-from').value, filter_end: document.getElementById('filter-created-until').value }).then(res => {
            let html = '';
            if (res.status === 'true') {
                let allowEdit = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'addons', 'edit', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                let allowDelete = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'addons', 'delete', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                res.response.forEach(item => {
                    let badge = item.status === 'active' ? 'op-badge-success' : (item.status === 'inactive' ? 'op-badge-danger' : 'op-badge-secondary');
                    let redirectEdit = allowEdit ? `style="cursor:pointer;" onclick="load_content('Edit Addons','<?php echo $site_url ?>admin/addons/edit?ref=${item.id}','nav-item-addons')"` : '';
                    let redirectDelete = allowDelete ? `onclick="deleteItem('${item.id}')"` : '';
                    html += `<tr data-id="${item.id}"><td><input class="op-checkbox rowCheckbox" type="checkbox"></td><td ${redirectEdit}><div class="font-medium">${item.name}</div></td><td ${redirectEdit}><span class="${badge}">${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</span></td>
    ...
    document.querySelector('.modal-createItem-btn').addEventListener('click', function(){
        ...
        var addon = modal.querySelector('select[name="addon"]').value;
        if(!addon){ apToast('error','Incomplete','Please select an addon.'); return; }
        var btnEl = this; var btn = btnEl.innerHTML;
        btnEl.innerHTML = '<div class="op-spinner" style="width:16px;height:16px;border-width:2px"></div>';
        opFetch('addons-create', { addon }).then(response => {

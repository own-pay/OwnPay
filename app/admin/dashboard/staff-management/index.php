...
<div class="op-page-header">
    <div><div class="op-page-pretitle">Settings</div><h2 class="op-page-title">Staff Members</h2></div>
    <div class="flex items-center gap-2">
        <span class="global-loaderSpinner"></span>
        <span onclick="load_content('Create Staff','<?php echo $site_url.$path_admin ?>/staff-management/create','nav-item-staff-management')" class="<?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'create', $global_user_response['response'][0]['role']) ? '' : 'hidden' ?>">
            <button class="op-btn-primary">Create Staff</button>
        </span>
    </div>
</div>
...
<div class="op-card">
    ...
    <div class="overflow-x-auto"><table class="op-table"><thead><tr><th class="w-8"><input class="op-checkbox select-all" type="checkbox"></th><th>Staff</th><th>Username</th><th>Role</th><th>Created Date</th><th>Status</th><th></th></tr></thead><tbody class="table-data-list"></tbody></table></div>
...
<div id="model-bulkAction" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" data-modal-backdrop="static">
    <div class="relative p-4 w-full max-w-md max-h-full"><div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800">
        ...
        <div class="p-4"><label class="op-label">Action <span class="text-red-500">*</span></label><select class="op-select" id="model-bulkActionID"><option value="" selected>Select a Action</option>
            <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'delete', $global_user_response['response'][0]['role']) ? '<option value="deleted">Delete Selected</option>' : '' ?>
            <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit', $global_user_response['response'][0]['role']) ? '<option value="activated">Activate Selected</option><option value="suspended">Suspend Selected</option>' : '' ?>
        </select></div>
...
<script nonce="<?= $csp_nonce ?? '' ?>" data-cfasync="false">
    ...
    document.querySelector('.model-bulkAction-btn').addEventListener('click', function(){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            opFetch('staff-bulk-action', { actionID, selected_ids: JSON.stringify(selectedRows) }).then(response => {
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            opFetch('staff-delete', { ItemID }).then(response => {
        ...
        } else { show_action_confirmation_tab('btnDeleteItem-'+ItemID, 'Delete Staff', 'Delete', 'btn-danger'); }
    }

    function load_data_list(page = 1){
        currentPage = page;
        document.querySelector(".table-data-list").innerHTML = apSkeletonRows(5);
        opFetch('staff-management-list', { search_input: document.querySelector('.search_input').value, show_limit: document.querySelector('.show_limit').value, page, filter_status: document.getElementById('filter-status').value, filter_start: document.getElementById('filter-created-from').value, filter_end: document.getElementById('filter-created-until').value }).then(res => {
            let html = '';
            if (res.status === 'true') {
                let allowEdit = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                let allowDelete = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'delete', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                let allowPermission = <?= \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'view_permission_list', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                res.response.forEach(item => {
                    let badge = item.status === 'active' ? 'op-badge-success' : (item.status === 'suspend' ? 'op-badge-danger' : 'op-badge-secondary');
                    let redirectEdit = allowEdit ? `style="cursor:pointer;" onclick="load_content('Edit Staff','<?php echo $site_url.$path_admin ?>/staff-management/edit?staff=${item.id}','nav-item-staff-management')"` : '';
                    let redirectDelete = allowDelete ? `onclick="deleteItem('${item.id}')"` : '';
                    let redirectPermission = allowPermission ? `style="cursor:pointer;" onclick="load_content('Permissions List','<?php echo $site_url.$path_admin ?>/staff-management/permissions-list?staff=${item.id}','nav-item-staff-management')"` : '';
                    html += `<tr data-id="${item.id}"><td><input class="op-checkbox rowCheckbox" type="checkbox"></td><td ${redirectEdit}><div class="font-medium">${item.name}</div><div class="text-sm text-gray-500">${item.email}</div></td><td ${redirectEdit}>${item.username}</td><td ${redirectEdit}>${item.role.charAt(0).toUpperCase() + item.role.slice(1)}</td><td ${redirectEdit}>${item.created_date}</td><td ${redirectEdit}><span class="${badge}">${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</span></td>
                    <td class="text-end"><button class="op-btn-secondary text-xs" data-dropdown-toggle="dd-${item.id}">Actions</button><div id="dd-${item.id}" class="hidden z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700"><ul class="py-2 text-sm"><li class="${allowEdit?'':'hidden'}"><a href="javascript:void(0)" ${redirectEdit} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">Edit</a></li><li class="${allowPermission?'':'hidden'}"><a href="javascript:void(0)" ${redirectPermission} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">Permissions</a></li><li class="${allowDelete?'':'hidden'}"><a href="javascript:void(0)" ${redirectDelete} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 btnDeleteItem-${item.id}">Delete</a></li></ul></div></td></tr>`;
                });

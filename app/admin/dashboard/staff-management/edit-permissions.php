...
<form class="form-staff-management-update">
    ...
    <div class="op-card"><div class="p-4"><div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div><label class="op-label">Status <span class="text-red-500">*</span></label><select class="op-select" name="status"><option value="active" <?= $response_permission['response'][0]['status'] === 'active' ? 'selected' : '' ?>>Active</option><option value="suspend" <?= $response_permission['response'][0]['status'] === 'suspend' ? 'selected' : '' ?>>Suspend</option></select></div>
    ...
    <div class="op-card mt-4">
        <div class="border-b border-gray-200 dark:border-gray-700"><ul class="flex flex-wrap -mb-px text-sm font-medium text-center" role="tablist">
            ...
            foreach($schema as $tabKey=>$tabData): $tabId='tab-'.$tabKey; $totalCount=countPermissions($tabKey,$tabData); ?>
                <li class="me-2" role="presentation"><button class="inline-block p-4 border-b-2 rounded-t-lg <?= $i===0?'border-primary-600 text-primary-600':'border-transparent hover:text-gray-600' ?>" data-tabs-target="#<?=$tabId?>" type="button" role="tab"><?=ucfirst(str_replace('_',' ',$tabKey))?> <span class="bg-primary-100 text-primary-800 text-xs font-medium ms-1 px-2 py-0.5 rounded-full"><?=$totalCount?></span></button></li>
            <?php $i++; endforeach; ?>
        </ul></div>
        <div class="p-4">
            ...
            <div class="<?=$i===0?'':'hidden'?>" id="<?=$tabId?>" role="tabpanel">
                ...
                <div class="border rounded-lg p-4 mb-3 dark:border-gray-700">
                    <div class="flex justify-between items-center border-b pb-2 mb-3"><span class="font-semibold text-sm"><?=ucfirst(str_replace(['_','-'],' ',$module))?></span><span onclick="select_by_box('<?=$rid?>')" class="btn-<?=$rid?> text-primary-600 cursor-pointer text-sm">Select All</span></div>
                ...
                <div class="mb-3"><span onclick="select_by_box('<?=$rid?>')" class="btn-<?=$rid?> text-primary-600 cursor-pointer text-sm">Select All</span></div>

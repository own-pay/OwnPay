<?php
    ...
    if (!\OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit', $global_user_response['response'][0]['role'])) { http_response_code(403); exit('Access denied.'); }
    ...
    $canEditPermission = \OwnPay\Service\Auth\PermissionService::hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit_permission', $global_user_response['response'][0]['role']);
    if ($canEditPermission) {
        $response_perm = json_decode(\OwnPay\Service\System\CrudService::selectLegacy($db_prefix.'permission','WHERE a_id = :a_id', '* FROM', [':a_id' => $staff_id]),true);

        ...
        extract($context);
                    ...
                    if ($api_type == "checkout") {
                        ...
                        if ($checkout_type == "redirect") {
                            $fullName = InputSanitizer::html($data['full_name'] ?? '');
                            $email = InputSanitizer::html($data['email_address'] ?? '');
                            $mobile = InputSanitizer::html($data['mobile_number'] ?? '');
                            $amount = $data['amount'] ?? '0';
                            $currency = InputSanitizer::html($data['currency'] ?? 'BDT');
                            ...
                            */
                            ...
                            if ($idemKey !== null && $idemKey !== '' && class_exists('\OwnPay\Service\Payment\IdempotencyBridge')) {
                                $idemBridge = new \OwnPay\Service\Payment\IdempotencyBridge($db_prefix);
                        ...
                        if ($api_type == "verify-payment") {
                            ...
                            if ($op_id == "") {
                                ...
                                $params = [':ref' => $op_id, ':brand_id' => $response_api['response'][0]['brand_id']];

                                $response_transaction = CrudService::select($db_prefix . 'transaction', 'WHERE ref = :ref AND brand_id = :brand_id', '* FROM', $params);

...
detect_os() {
  ...
  case "$OS_ID" in
    ...
    almalinux|rocky|centos|rhel)
      ...
      case "${OS_VERSION%%.*}" in
        8|9) ;;
        *) log_warn "${OS_ID^} $OS_VERSION is untested. Proceeding anyway." ;;
      esac
      ;;
    *)
      log_error "Unsupported OS: $OS_ID. OwnPay installer supports Ubuntu, Debian, AlmaLinux, Rocky Linux, and CentOS."
      exit 1
      ;;
  esac
}
...
run_quiet() {
  local desc="$1"; shift
  spinner_start "$desc"
  # Always suppress apt/dnf interactive prompts regardless of caller
  if [ "${1:-}" = "apt-get" ] || [ "${1:-}" = "apt" ]; then
    DEBIAN_FRONTEND=noninteractive "$@" >> "$LOG_FILE" 2>&1
  elif [ "${1:-}" = "dnf" ] || [ "${1:-}" = "yum" ]; then
    "$@" -y -q >> "$LOG_FILE" 2>&1
  else
    "$@" >> "$LOG_FILE" 2>&1
  fi
...
pkg_install() {
  if [ "$OS_FAMILY" = "debian" ]; then
    ...
    $PKG_MGR install -y -q "$@" >> "$LOG_FILE" 2>&1
  fi
}
...
pkg_update_index() {
  if [ "$OS_FAMILY" = "debian" ]; then
    ...
    run_quiet "Updating package index" $PKG_MGR makecache -q
  fi
}
...
https://packages.sury.org/php/ ${OS_CODENAME} main" \
    ...
    # Add Remi repository for RHEL 8/9
    local remi_url="https://rpms.remirepo.net/enterprise/remi-release-${OS_VERSION%%.*}.rpm"
    spinner_start "Adding Remi PHP repository..."
    $PKG_MGR install -y -q "$remi_url" >> "$LOG_FILE" 2>&1 || true
    # Reset and enable module
    $PKG_MGR module reset php -y -q >> "$LOG_FILE" 2>&1 || true
    $PKG_MGR module enable php:remi-8.3 -y -q >> "$LOG_FILE" 2>&1 || true
    spinner_stop
    log_success "Remi PHP repository configured for PHP 8.3"
  fi
  ...
  if [ "$OS_FAMILY" = "debian" ]; then
    ...
    PHP_SVC="php8.3-fpm"
    opcache_ini="/etc/php/8.3/fpm/conf.d/10-opcache.ini"
  else
    php_extensions=(
      "php" "php-fpm" "php-cli"
      "php-mysqlnd" "php-pdo"
  ...
  local php_ini
  local ini_paths=( "/etc/php/8.3/fpm/php.ini" "/etc/php/8.3/cli/php.ini" "/etc/php.ini" )
  for ini_path in "${ini_paths[@]}"; do
    if [ -f "$ini_path" ]; then
...
_install_nginx() {
  ...
  if [ "$OS_FAMILY" = "rhel" ]; then
    vhost_file="/etc/nginx/conf.d/ownpay.conf"
  fi
...
_install_apache() {
  ...
  local apache_log="/var/log/apache2"
  local vhost_file="/etc/apache2/sites-available/ownpay.conf"

  if [ "$OS_FAMILY" = "rhel" ]; then
    apache_svc="httpd"
    apache_log="/var/log/httpd"
    vhost_file="/etc/httpd/conf.d/ownpay.conf"
  fi

  if ! cmd_exists "$apache_svc"; then
    pkg_install "$apache_svc"
    log_success "Installing ${apache_svc^}"
  else
    log_success "${apache_svc^} already installed"
  fi
...
<VirtualHost *:80>
    ...
    ErrorLog ${apache_log}/ownpay-error.log
    CustomLog ${apache_log}/ownpay-access.log combined
</VirtualHost>
APACHE
  ...
  run_quiet "Restarting ${apache_svc^}" systemctl restart "$apache_svc"
  run_quiet "Enabling ${apache_svc^} on boot" systemctl enable "$apache_svc"
  log_success "${apache_svc^} configured for ${DOMAIN}"
}
...
_install_caddy() {
  if ! cmd_exists caddy; then
    ...
    if [ "$OS_FAMILY" = "debian" ]; then
      ...
      $PKG_MGR install -y 'dnf-command(copr)' >>"$LOG_FILE" 2>&1 || true
      $PKG_MGR copr enable @caddy/caddy -y >>"$LOG_FILE" 2>&1 || true
    fi
    spinner_stop
    pkg_update_index
    pkg_install caddy
...
phase_redis() {
  ...
  # Configure Redis
  sed -i 's/^# maxmemory-policy.*/maxmemory-policy allkeys-lru/' /etc/redis/redis.conf 2>/dev/null || \
  sed -i 's/^# maxmemory-policy.*/maxmemory-policy allkeys-lru/' /etc/redis.conf 2>/dev/null || true

  local redis_svc="redis-server"
  [ "$OS_FAMILY" = "rhel" ] && redis_svc="redis"

  run_quiet "Restarting Redis" systemctl restart "$redis_svc"
  run_quiet "Enabling Redis on boot" systemctl enable "$redis_svc"
  run_quiet "Restarting PHP-FPM" systemctl restart "$PHP_SVC"
...
phase_firewall() {
  show_phase_header "12" "🛡️" "Firewall" "Hardening server with firewall rules"
  ...
  if [ "$OS_FAMILY" = "debian" ]; then
    ...
    if ! cmd_exists firewall-cmd; then
      ...
      log_success "Installing Firewalld"
    fi

    run_quiet "Enabling Firewalld" systemctl enable firewalld
    run_quiet "Starting Firewalld" systemctl start firewalld
    ...
    log_success "Firewalld enabled: SSH + HTTP + HTTPS allowed"
    log_warn "All other ports are blocked by default"
    
    # Fail2ban
    if $PKG_MGR info fail2ban > /dev/null 2>&1; then
      pkg_install fail2ban
      log_success "Installing Fail2ban"
      systemctl enable fail2ban >> "$LOG_FILE" 2>&1 || true
      systemctl start fail2ban >> "$LOG_FILE" 2>&1 || true

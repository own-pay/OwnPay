...
| **HIGH** | 4 | FIND-001, FIND-005, FIND-016, FIND-019 |
...
---
### [HIGH] [Mobile API] FIND-019 — Device pairing bootstrap blocked by JWT middleware (added during Deliverable 3)
- **Severity**: HIGH
- **Quest**: UI-UX Framework / Mobile API (Quest 10)
- **Dimension**: Authorization over-restriction breaking a core bootstrap flow
- **Location**: `config/routes/api.php:40` (pair route → `mobile` group), `config/middleware.php:72-76` (`mobile` includes `JwtAuthMiddleware`), `src/Middleware/JwtAuthMiddleware.php:47-54`.
- **Description**: `JwtAuthMiddleware` returns 401 when no Bearer token is present and has **no route allowlist**, yet `POST /api/mobile/v1/devices` (pairing) is in the JWT-guarded `mobile` group. A new device has no token (it authenticates with the OTP in the body), so pairing is rejected with 401 before `DeviceController::pair` runs — **no device can complete initial pairing as wired**. (Token-refresh works only if the client sends a still-valid refresh JWT as the bearer.)
- **Evidence**:
```php
// JwtAuthMiddleware.php:47-54
$token = $request->bearerToken();
if ($token === null || $token === '') {
    return Response::json(['success'=>false,'message'=>'JWT token required'], 401);
}
// config/routes/api.php:40  → 'mobile' (JWT-guarded) group
$router->post('/api/mobile/v1/devices', 'Api\\Mobile\\DeviceController@pair', 'mobile');
```
- **Impact**: Companion-app onboarding is non-functional; the SMS-automation feature cannot be activated. Release-blocking for the mobile integration (sibling defect to FIND-003).
- **Proposed Technical Fix**: route pairing (and token-refresh) through a JWT-free bootstrap group (OTP/refresh-JWT-in-body authenticate them), or add a path allowlist to `JwtAuthMiddleware`:
```php
...
```
- **Technical Decision**: Pairing is already authenticated by the single-use `FOR UPDATE` OTP; refresh by the refresh JWT in the body. Keeping them in the JWT group is an over-restriction, not added safety. Full detail in `mobile_architecture.md` §1.3.
- **Compliance Tag**: CWE-285 (Improper Authorization); OWASP-A04.

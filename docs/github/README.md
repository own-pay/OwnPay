...
With native support for over 120+ payment gateways, the **world's first self-hosted white-label custom-domain checkout architecture**, a secure double-entry ledger, and a sandboxed plugin engine, OwnPay gives you the power of an enterprise payment gateway on your own private server.
...
### 🏷️
**White-Label — Industry First**

The only self-hosted platform where every brand runs on its own custom domain. Your customers see YOUR logo, name, and colors — never OwnPay. One server, unlimited brands, zero trace.
...
#### 🏷️ White-Label — Industry First
- **First self-hosted platform** with per-brand custom domain checkout
- One install → unlimited brands, each on its own domain
- Customers see YOUR logo, name & colors — not OwnPay
- Full data isolation — brands are blind to each other
...
```mermaid
%%{init: { 'theme': 'base', 'themeVariables': { 'primaryColor': '#0066FF', 'primaryTextColor': '#ffffff', 'primaryBorderColor': '#0044CC', 'actorBkg': '#0066FF', 'actorTextColor': '#ffffff', 'actorBorderColor': '#0044CC', 'signalColor': '#9CA3AF', 'signalTextColor': '#0066FF', 'noteBkgColor': '#EFF6FF', 'noteTextColor': '#1E3A5F', 'noteBorderColor': '#93C5FD' } } }%%
sequenceDiagram
    autonumber
    actor  Store    as 🏪 Your Store
    participant OwnPay  as ⚙️ OwnPay
    actor  Customer as 👤 Customer
    participant Gateway as 💳 Payment Gateway

    Store    ->> OwnPay  : Create a payment
    OwnPay   ->> Customer: Show your branded checkout page
    Note over OwnPay, Customer: Customer sees YOUR brand - not OwnPay
    Customer ->> Gateway : Pay via card, wallet, bank, or mobile
    Gateway -->> OwnPay  : Payment verified & confirmed
    OwnPay  -->> Store   : Transaction complete - ledger updated ✅
    Note over Store, OwnPay: Your data stays on your server. Always.
```

**That's it.** OwnPay runs silently on your server between your store and the payment provider. Your customer sees your brand. Your data never leaves your machine. You get a verified result and a permanent ledger record — no middlemen, no third-party access, no SaaS fees.

> Want the technical deep-dive? Read the **[Architecture Guide →](docs/ARCHITECTURE.md)**
...
cd /var/www
curl -L -o ownpay.zip https://github.com/own-pay/OwnPay/releases/latest/download/ownpay-x.y.z.zip
...
<div align="center"><sub>📖 Full production deployment, hardening & scaling guides → <a href="https://learn.ownpay.org">learn.ownpay.org</a></sub></div>
...
| 📖 **API Reference** | [docs.ownpay.org](https://docs.ownpay.org) | REST API schema & reference |
| 🎓 **Learn / Guides** | [learn.ownpay.org](https://learn.ownpay.org) | Step-by-step tutorials, how-tos, deployment & integration |
| 🧩 **Plugins** | [plugin.ownpay.org](https://plugin.ownpay.org) | Gateway & add-on catalog |
...
| 📋 **Feature Reference** | [docs/FEATURES.md](docs/FEATURES.md) | Every OwnPay capability documented in one place |

**In this repository:** [Feature Reference](docs/FEATURES.md) · [Architecture](docs/ARCHITECTURE.md) · [Local Setup](docs/LOCAL_SETUP.md) · [Translations](docs/TRANSLATIONS.md) · [Contributing](CONTRIBUTING.md) · [Roadmap](ROADMAP.md) · [Security](SECURITY.md) · [Support](SUPPORT.md) · [Governance](GOVERNANCE.md) · [Code of Conduct](CODE_OF_CONDUCT.md)
...
It's stable and usable today. For production, pin to a tagged release, test in staging first, keep backups, and follow the hardening and deployment guides at [learn.ownpay.org](https://learn.ownpay.org). OwnPay handles real money — correctness and security are the top priorities on the road to 1.0, and your [issue reports](https://github.com/own-pay/OwnPay/issues) directly shape it.
...
<a href="https://github.com/own-pay/OwnPay/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=own-pay/OwnPay" alt="OwnPay Contributors" />
</a>
...
[![Community Group](https://img.shields.io/badge/Community-Join%20Group-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/groups/ownpay.org)&nbsp;
[![Email](https://img.shields.io/badge/Email-ping%40ownpay.org-0066FF?style=for-the-badge)](mailto:ping@ownpay.org)&nbsp;
[![Donate](https://img.shields.io/badge/%E2%9D%A4%EF%B8%8F%20Donate-ownpay.org%2Fdonate-FF6B6B?style=for-the-badge)](https://ownpay.org/donate)
...
<a href="https://star-history.com/#own-pay/OwnPay&Date">
  <picture>
    ...
    <img width="90%" src="https://api.star-history.com/svg?repos=own-pay/OwnPay&type=Date" alt="Star History Chart" />
  </picture>
</a>

...
* **Bug 2.2:** No delete brand option. Brand index page has no delete button/action. Admin cannot remove brands.
...
* **Bug 11.1:** Domain verification checks TXT record but should check A record. Custom domains used for customer-facing pages (checkout, invoice, payment link) need DNS A pointing to server IP. **[DomainController rewritten, verify logic needs update]**
...
* **Bug 13.1:** Gateway page UI/UX not professional enough for fintech standard.
* **Bug 13.2:** After activating a gateway plugin, button still shows "Activate" instead of changing state.
...
## 15. Invoice Issues
* **Bug 15.1:** Invoice status always shows "draft" regardless of actual payment state.

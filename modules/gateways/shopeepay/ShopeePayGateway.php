...
/**
 * ShopeePay payment gateway adapter using Omise hosted payment sources.
 */
final class ShopeePayGateway implements PluginInterface, GatewayAdapterInterface, TestableConnectionInterface
{
    use GatewayDefaults;
    ...
    public function slug(): string { return 'shopeepay'; }
    public function name(): string { return 'ShopeePay'; }
    public function version(): string { return '1.0.0'; }
    public function description(): string { return 'ShopeePay e-wallet integration via Omise hosted sources'; }
    ...
    public function initiate(array $params, array $credentials): array
    {
        ...
        if (empty($secretKey)) {
            throw new \RuntimeException('ShopeePay error: Missing Omise Secret Key.');
        }
        ...
        // Convert amount to smallest subunit (Satang for THB or cents for MYR - 2 decimal places)
        $amountVal = $params['amount'];
        if (!is_numeric($amountVal)) {
            throw new \RuntimeException('ShopeePay error: Invalid transaction amount.');
        }
        $amountSubunits = (int) bcmul((string)$amountVal, '100', 0);

        // Step 1: Create Omise Source for ShopeePay
        $ch = curl_init(self::API_URL . '/sources');
        if ($ch === false) {
            throw new \RuntimeException('ShopeePay cURL initialization failed.');
        }
        ...
        if ($httpCode >= 400 || $response === false) {
            ...
            throw new \RuntimeException('ShopeePay Source creation failed [' . $httpCode . ']: ' . $msg);
        }

        $sourceData = json_decode((string) $response, true);
        if (!is_array($sourceData)) {
            throw new \RuntimeException('ShopeePay invalid source API response');
        }

        $sourceId = $this->getString($sourceData['id'] ?? '');
        if (empty($sourceId)) {
            throw new \RuntimeException('ShopeePay error: Missing source ID in response.');
        }

        // Step 2: Create Omise Charge with ShopeePay Source
        $chCharge = curl_init(self::API_URL . '/charges');
        if ($chCharge === false) {
            throw new \RuntimeException('ShopeePay Charge cURL initialization failed.');
        }
        ...
        if ($httpCodeCharge >= 400 || $responseCharge === false) {
            ...
            throw new \RuntimeException('ShopeePay Charge creation failed [' . $httpCodeCharge . ']: ' . $msg);
        }

        $chargeData = json_decode((string) $responseCharge, true);
        if (!is_array($chargeData)) {
            throw new \RuntimeException('ShopeePay invalid charge API response');
        }
    ...
    public function verify(array $callbackData, array $credentials): array
    {
        ...
        if (empty($chargeId)) {
            return [
                ...
                'gateway_trx_id' => '',
                'status'         => 'failed',
                'error'          => 'ShopeePay verification error: Missing charge identifier.',
            ];
        }
        ...
        if ($ch === false) {
            return [
                ...
                'gateway_trx_id' => '',
                'status'         => 'failed',
                'error'          => 'ShopeePay cURL initialization failed during status query.',
            ];
        }
        ...
        if ($httpCode >= 400 || $response === false) {
            return [
                ...
                'gateway_trx_id' => '',
                'status'         => 'failed',
                'error'          => 'ShopeePay status lookup failed with HTTP code ' . $httpCode,
            ];
        }
        ...
        if (!is_array($data)) {
            return [
                ...
                'gateway_trx_id' => '',
                'status'         => 'failed',
                'error'          => 'ShopeePay returned invalid JSON status.',
            ];
        }
    ...
    public function testConnection(array $credentials): array
    {
        ...
        $data = json_decode((string) $response, true);
        if ($httpCode === 200) {
            $mode = str_starts_with($secretKey, 'skey_test') ? 'sandbox' : 'live';
            return ['success' => true, 'message' => "Connected successfully to Omise/ShopeePay ({$mode} mode)."];
        }

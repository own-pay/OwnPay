...
/**
 * Trust Axiata Pay (Tap) mobile financial services and digital checkout gateway adapter.
 */
final class TapGateway implements PluginInterface, GatewayAdapterInterface
{
    use GatewayDefaults;

    private const SANDBOX_URL = 'https://tapapi-test.trustaxiatapay.com';
    private const LIVE_URL    = 'https://tapapi.trustaxiatapay.com';
    ...
    {
        return 'Trust Axiata Pay (Tap) mobile financial services and digital checkout integration';
    }
    ...
    public function initiate(array $params, array $credentials): array
    {
        ...
        if ($merchantId === '' || $apiKey === '' || $secretKey === '') {
            throw new \RuntimeException('Tap error: Missing Merchant ID, API Key, or Secret Key.');
        }
        ...
        // Generate SHA256 hash for secure token exchange
        ...
        if ($ch === false) {
            throw new \RuntimeException('Tap: Failed to initialize curl.');
        }
        ...
        if ($httpCode !== 200 || $response === false) {
            if ($mode === 'live') {
                throw new \RuntimeException('Tap connection error: ' . ($response ?: 'Empty API response'));
            }
        ...
        if (is_array($resData)) {
            $checkoutUrl = $this->getString($resData['checkout_url'] ?? null);
            if ($checkoutUrl !== '') {
        ...
        throw new \RuntimeException('Tap error: Failed to retrieve redirect URL');
    }
    ...
    public function verify(array $callbackData, array $credentials): array
    {
        ...
        // Direct server-side API checkback check to fully secure verification
        $baseUrl = $mode === 'live' ? self::LIVE_URL : self::SANDBOX_URL;

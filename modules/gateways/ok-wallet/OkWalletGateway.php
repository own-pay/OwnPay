 ...
 */
final class OkWalletGateway implements PluginInterface, GatewayAdapterInterface, TestableConnectionInterface
{
    use GatewayDefaults;
    ...
    /**
     * Verifies the configured Merchant ID/API Key/Secret Key authenticate against the OK Wallet
     * API by attempting a signed status lookup with a dummy transaction reference - reuses the
     * same base URL, signature scheme, and header style as verify(). A structured JSON response
     * confirms the signature was accepted; a network failure or auth-rejection status does not.
     *
    ...
    public function testConnection(array $credentials): array
    {
        ...
        if ($response === false) {
            return ['success' => false, 'message' => 'Could not reach the OK Wallet API - check the server\'s network connectivity.'];
        }
        if ($httpCode === 401 || $httpCode === 403) {
            return ['success' => false, 'message' => 'OK Wallet rejected the provided Merchant ID/Secret Key.'];
        }
        return ['success' => true, 'message' => "Reached the OK Wallet API and credentials were accepted ({$mode} mode)."];
    }
